...
namespace FindIt.Domain.Utilities
{
	public class RoundaboutPrefabCategoryProcessor : IPrefabCategoryProcessor
	{
		...
		public bool TryCreatePrefabIndex(PrefabBase prefab, Entity entity, out PrefabIndex prefabIndex)
		{
			...
			{
				prefabIndex.SubCategory = Enums.PrefabSubCategory.Networks_Intersections;
			}
			else if (_entityManager.HasComponent<PillarData>(entity))
			{
				prefabIndex.Category = Enums.PrefabCategory.Props;
				prefabIndex.SubCategory = Enums.PrefabSubCategory.Props_Road;
			}
			else
			{
				prefabIndex.SubCategory = Enums.PrefabSubCategory.Networks_Stops;
			}
